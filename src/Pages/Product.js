...
function Product() {
  ...
  let similarProducts = products
    .filter(
      (item) => item.id !== parseInt(id) && item.category === product.category
    )
    .slice(0, 3);
  return (
    <Fragment>
      <Container>
        <Row className="justify-content-between align-items-center">
          ...
          <Col xs="7" className="ps-4">
            <h2 style={{ textAlign: "left" }} className="mb-3">
              {title}
            </h2>
            ...
            <div className="justify-content-between align-items-center d-flex w-100 m-auto mb-3">
              <div>
                {" "}
                <span className="text-success">&#9733;</span> {rate}
              </div>
              ...
              >
                ...
                >
                  <img
                    src={image}
                    alt={title}
                    className={`w-75 h-50 ${styles3.product_img}`}
                  />
                ...
                <Link className="w-100 " to={`/products/${product.id}`}>
                  <Button variant="primary" className="w-75 text-center">
                    Check Out
                  </Button>
                </Link>
              </Col>
