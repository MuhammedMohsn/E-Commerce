...
function HotDeals({ setProducts, products, setLoading,inputSearch}) {
  useEffect(() => {
  ...
  return (
    <Fragment>
      <Container className="mt-4">
        ...
        <Row>
          { (inputSearch||" ").length>0?products.filter((product)=>{return product.title.includes(inputSearch)}).map(product => {
